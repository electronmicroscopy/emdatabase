...
def _make_settings_class():
    ...
    class SettingsWidget(anywidget.AnyWidget):
        """An editable panel for emdatabase's settings (the data directory)."""
...
def settings_widget():
    """Return an interactive widget for editing emdatabase's settings (Jupyter).

    Backs ``display(emdatabase.settings)`` and ``emdatabase.settings.widget()``.
    Requires anywidget (``pip install emdatabase[widget]``).
    """
    _prepare_frontend()
